...
class Simulation():
    ...
    def sortie_sort(self, in_ytp, ftc_ytp, cf, period, P, M, waves): #TODO refactor live vars (x, x_a and ftc)
        ...
        _l = in_ytp[period] + ftc_ytp[period]
    ...
    def step(self, x, ftc, s, ftcs):
        sys.stdout.write('\r')
        sys.stdout.write('\n%i / %i completed ->'  %(self.period, self.cf.tau))

        #get events for day by constructing schedule for the day
        not_live_pilots, schedule = self.sortie_sort(x, ftc, self.cf, self.period, self.P, self.M, self.cf.waves)
        not_sim_pilots, schedule_sim = self.sortie_sort(s, ftcs, self.cf, self.period, self.P, self.M, self.cf.sim_waves)

...
class Simulation():
    ...
    def write_to_history(self, assigned_sorties):
        if self.cf.verbose:
    ...
    def step(self, x, x_a, ftc):
        ...
        idle_pilots, schedule = self.sortie_sort(x, x_a, ftc, self.cf, self.period, self.P, self.M)
        if self.cf.verbose:
        ...
        if self.period in self.cf.rescheduling:
            ...
            x_partial, x_a_partial, ftc_partial, s, s_a, ftcs, O, F = mip(
                    sim.history.P_live,
        ...
        if sum([len(schedule[x]) for x in schedule.keys()]) != 0 and self.svc[self.period] != 0:
            ...
            for k in schedule.keys(): #TODO use AC_day input instead of svc[period]
                if np.random.random() > self.cf.weater_abort and self.svc[self.period] != 0:
                    ...
                    self.write_to_history(assigned_sorties)

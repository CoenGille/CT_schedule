...
def mip(p_live, ftc_live, F_count, p_sim, ftc_sim, Fs_count, \
    ...
    gradient = np.linspace(1, 0, num=(min(90, cf.tau - period)))#np.full((min(30, cf.tau - period)), 0.5)
    ...
    #--------------------ALPHA CONSTRAINTS-------------
    # a pilot is either flying or in a sim not both                             [EQ2]
    for p in P:
        for t in T:
    ...
    for p in P:
        ...
        #                                                                       [EQ5 EQ6]
        model += cf.FTC_max[P[(p)][1]-1] + math.ceil((cf.FTC_extra[P[p][1]-1] * \
            (cf.tau-period)/cf.tau)) - ftc_count[p] +2 >= \
            pl.lpSum(FTC[(p,t,m)] for t in T for m in M)    
                     
        model += cf.FTC_max[P[(p)][3]-1] + math.ceil((cf.FTC_extra[P[p][3]-1] * \
            (cf.tau-period)/cf.tau)) - ftcs_count[p] +2 >= \
            pl.lpSum(FTC_s[(p,t,m)] for t in T for m in M)    
    
    #--------------------BETA CONSTRAINTS-------------
    # force aux var F to indicate mission execution                             [EQ7 EQ8]     
    for t in T:
    ...
    # A mission has certified flight leads                                      [EQ9 EQ10]   
    for t in T:
        for m in M2:
    ...
    #disallow night missions during summer period                               [EQ13]
    for t in T:
    ...
    # max number of flights is determined by the FMP, sim sorties 
    # are based on simulator availabilty                                        [EQ14 EQ15]                                                            
    for t in T:

...
def mip(A, cf, M, M2, M4, P, period, Pfl2, Pfl4, QRA, sorties, *args, **kwargs):
    ...
    for p in P:
        for t in T:
            model += pl.lpSum(X[(p,t,m)] for m in M) + pl.lpSum(S[(p,t,m)] for m in M) + \
                     O[(p,t)] + pl.lpSum(X_a[(p,t,m)] for m in M) + \
                     pl.lpSum(S_a[(p,t,m)] for m in M) + pl.lpSum(FTC[(p,t,m)] for m in M) \
                     == A[(p)][t]
        ...
        for m in M:
            if M[m][0] == 2:
                model += F[(t,m)] <= pl.lpSum(X[(p,t,m)] for p in Pfl2) + \
                         pl.lpSum(X_a[(p,t,m)] for p in Pfl2) + pl.lpSum(FTC[(p,t,m)] for p in Pfl2)
                model += F_s[(t,m)] <= pl.lpSum(S[(p,t,m)] for p in Pfl2) + \
                         pl.lpSum(S_a[(p,t,m)] for p in Pfl2) + pl.lpSum(FTC_s[(p,t,m)] for p in Pfl2)
                
    for t in T:
        for m in M:
            if M[m][0] == 4:
                model += F[(t,m)] <= pl.lpSum(X[(p,t,m)] for p in Pfl4) + \
                         pl.lpSum(X_a[(p,t,m)] for p in Pfl4) + pl.lpSum(FTC[(p,t,m)] for p in Pfl4)  
                model += F_s[(t,m)] <= pl.lpSum(X[(p,t,m)] for p in Pfl4) + \
                         pl.lpSum(S_a[(p,t,m)] for p in Pfl4) + pl.lpSum(FTC_s[(p,t,m)] for p in Pfl4)   
                
    #--------------------BETA CONSTRAINTS-------------
            
    # max number of flights is max AC
    for t in T:
        model += pl.lpSum(X[(p,t,m)] for p in P for m in M) + pl.lpSum(X_a[(p,t,m)] for p in P for m in M) \
                 + pl.lpSum(FTC[(p,t,m)] for p in P for m in M) <= sorties[t]
        model += pl.lpSum(S[(p,t,m)] for p in P for m in M) + pl.lpSum(S_a[(p,t,m)] for p in P for m in M) + \
    ...
    for p in P:
        ...
        model += cf.FTC_max[P[(p)][2]-1] <= pl.lpSum(FTC_s[(p,t,m)] for t in T for m in M) \
                 <= cf.FTC_max[P[(p)][3]-1]
        for m in M:
            model += M[m][P[(p)][0]] <= pl.lpSum(X[(p,t,m)] for t in T) <= M[m][P[(p)][1]]
            model += M[m][P[(p)][2]] <= pl.lpSum(S[(p,t,m)] for t in T) <= M[m][P[(p)][3]]


    # force aux var F to indicate mission execution      
    for t in T:
        model += F[(t,'BFM')] == pl.lpSum(X[(p,t,'BFM')] for p in P) + pl.lpSum(FTC[(p,t,'BFM')] for p in P)
        model += F_s[(t,'BFM')] == pl.lpSum(S[(p,t,'BFM')] for p in P) + pl.lpSum(FTC_s[(p,t,'BFM')] for p in P)
        for m in M2:
            model += F[(t,m)] * 2 == pl.lpSum(X[(p,t,m)] for p in P) + pl.lpSum(X_a[(p,t,m)] for p in P) + \
                     pl.lpSum(FTC[(p,t,m)] for p in P)
            model += F_s[(t,m)] * 2 == pl.lpSum(S[(p,t,m)] for p in P) + pl.lpSum(S_a[(p,t,m)] for p in P) + \
                     pl.lpSum(FTC_s[(p,t,m)] for p in P)
        for m in M4:
            model += F[(t,m)] * 4 == pl.lpSum(X[(p,t,m)] for p in P) + pl.lpSum(X_a[(p,t,m)] for p in P) + \
                     pl.lpSum(FTC[(p,t,m)] for p in P)
            model += F_s[(t,m)] * 4 == pl.lpSum(S[(p,t,m)] for p in P) + pl.lpSum(S_a[(p,t,m)] for p in P) + \
                     pl.lpSum(FTC_s[(p,t,m)] for p in P)
    ...
    #--------------------WRITE SOLUTION-------------------
    #TODO write func to anutomate this horrible data formatting

...
class Simulation():
    ...
    def sortie_sort(self, in_ytp, out_ytp, ftc_ytp, cf, period, P, M, waves): #TODO refactor live vars (x, x_a and ftc)
        ...
        _l = in_ytp[period] + out_ytp[period] + ftc_ytp[period]
    ...
    def write_live_to_history(self, assigned_sorties, ftc, period): #TODO add FTC counts
        if self.cf.verbose:
    ...
    def step(self, x, x_a, ftc, s, s_a, ftcs):
        ...
        not_live_pilots, schedule = self.sortie_sort(x, x_a, ftc, self.cf, self.period, self.P, self.M, self.cf.waves)
        not_sim_pilots, schedule_sim = self.sortie_sort(s, s_a, ftcs, self.cf, self.period, self.P, self.M, self.cf.sim_waves)
        ...
        #---------------------------- SIM MISSIONS ----------------------------    
        if sum([len(schedule_sim[x]) for x in schedule_sim.keys()]) != 0 and self.svc[self.period] != 0:
            for k in schedule_sim.keys():
                self.write_sim_to_history(schedule_sim[k])
    ...
    def run(self):
        ...
        if self.cf.init_schedule:
            ...
            x, x_a, ftc, s, s_a, ftcs = schedule_init()

        while self.period < self.cf.tau:
            try:
                x, x_a, ftc, s, s_a, ftcs = self.step(x, x_a, ftc, s, s_a, ftcs)
        ...
        for p in self.P:
            CT_complete[p] = 1- ((-1*(sum(sim.history.P_live[p][m] for m in self.M))) / (sum(self.M[m][self.P[p][0]] for m in self.M)))

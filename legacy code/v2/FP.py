...
def mip(p_live, ftc_live, ftc_count, p_sim, A, cf, M, M2, M4, P, period, Pfl2, Pfl4, QRA, sorties, *args, **kwargs):
    ...
    model += C == pl.lpSum(X[(p,t,m)] for p in P for t in T for m in M) + \
                  ...
                  0.2 * pl.lpSum(X_a[(p,t,m)] for p in P for t in T for m in M)- \
                  0.3 * pl.lpSum(S_a[(p,t,m)] for p in P for t in T for m in M)
    ...
    for p in P:
        model += cf.FTC_max[P[(p)][0]-1] <= pl.lpSum(FTC[(p,t,m)] for t in T for m in M) + ftc_count[p] \
                 <= cf.FTC_max[P[(p)][1]-1]
